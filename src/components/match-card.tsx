...
export function MatchCard({
  ...
  return (
    ...
    >
      ...
      >
        <div>
          <div className="mb-3 flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[.14em] text-muted">
            {live && <span className="pulse size-1.5 rounded-full bg-danger" />}
            {live
              ? `${match.elapsed ?? 0}' · Live`
              : match.status === "NS" || match.status === "TBD"
                ? matchTime(match.kickoff)
                : match.status_detail ?? match.status}
          </div>
          <div className="space-y-2 text-lg font-medium tracking-tight">
            <Team
...
function Team({
  ...
  return (
    <div className="flex min-w-0 items-center gap-3">
      <span className="grid size-7 shrink-0 place-items-center rounded-full bg-white/[.06] font-mono text-[9px] text-muted">
        {code?.slice(0, 3) ?? name.slice(0, 2).toUpperCase()}
      </span>
      <span className="truncate">{name}</span>
      {score !== null && (
        <span className="ml-auto font-mono text-xl">{score}</span>
      )}
    </div>
  );
}
