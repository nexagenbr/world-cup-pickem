...
import { fetchActiveFixtures, fetchAllFixtures } from "@/lib/football/api-football";
...
export async function syncTournament(type: "fixtures" | "results") {
  ...
  try {
    const fixtures = type === "fixtures"
      ? await fetchAllFixtures(env.footballApiKey())
      : await fetchActiveFixtures(env.footballApiKey());
